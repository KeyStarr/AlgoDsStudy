 ...
 *  • funny how one can solve DP problems left-to-right AND right-to-left but the optimal solution appears to be only in a single
 *   of these directions;
 *  • 🔥 as [HouseRobber] is, this problem is 1D and to each state depends on a static number of previous states
 *   => Fibonacci-like.
 *
 * Value gained:
...
class HouseRobberII {
    ...
    fun bottomUpFromRight(houses: IntArray): Int {
        ...
        dp[0][houses.lastIndex] = houses.last() // base case, last house, but we didn't take the first (if we did, its always 0)
